...
up establishing of connection and overall crawling.

To increase the maximum thread pool size use::
...
If you have multiple crawling processes and a single central DNS server, it can act
like a DoS attack on the DNS server, resulting in the entire network slowing down or
...
when sites are very slow (or fail) to respond, thus causing a timeout
...
crawler to dedicate too many resources on a specific domain.
...
Pages can indicate this in two ways:

1) by using ``#!`` in their URL - this is the default way;
...
:class:`AjaxCrawlMiddleware` helps to crawl them correctly.
